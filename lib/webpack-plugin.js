...
IsomorphicLoaderPlugin.prototype.apply = function(compiler) {
  ...
  const writeConfig = () => {
    ...
    return lockConfig.lock("write", () => {
      return fs.writeFile(configFile(), data, err => {
        try {
          if (err) {
            return logger.error(`isomorphic-loader: failed write config file ${configFile()}`, err);
          }
        } finally {
  ...
  const saveConfig = tag => {
    ...
    if (this.config.isWebpackDev) {
      ...
      configToSave.push({ valid: this.config.valid, timestamp: this.config.timestamp, data });
