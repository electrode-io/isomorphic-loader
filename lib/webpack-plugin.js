...
class IsomorphicLoaderPlugin {
  constructor(options) {
    this.config = { valid: false };
    this.options = deepExtend({ assetsFile: Config.defaultAssetsFile }, options);
    this.configFile = () => this.options.configFile || Path.resolve(Config.getCurrentConfigFile());
  ...
  apply(compiler) {
    ...
    const writeConfig = data => {
      ...
      if (this.config.isWebpackDev && process.send) {
        process.send({ name: Config.configName, config: data });
        return checkPending();
      }
