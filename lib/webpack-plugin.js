...
const { getWebpackRequest, removeLoaders, removeCwd, replaceAppSrcDir } = require("./utils");
...
class IsomorphicLoaderPlugin extends EventEmitter {
  ...
  apply(compiler) {
    ...
    const handleEmit = (compilation, callback) => {
      ...
      const loaderSigs = ["isomorphic-loader/lib/index.js!", "isomorphic-loader!"];

      const marked = stats.modules.reduce((acc, m) => {
        const foundSig = loaderSigs.find(sig => m.identifier.includes(sig));
