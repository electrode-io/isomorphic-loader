...
exports.getMyNodeModulesPath = (dir = __dirname) => {
  const myName = "isomorphic-loader";
  const ixName = dir.lastIndexOf(myName);

  if (ixName >= 0) {
    return posixify(dir.substr(ixName));
  }

  const parts = dir.split(Path.sep);
  // gather up to last 4 parts and join with posix path sep
  return parts.slice(Math.max(0, parts.length - 4)).join("/"); // eslint-disable-line
};
