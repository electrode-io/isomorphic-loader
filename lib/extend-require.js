...
ExtendRequire.prototype.interceptLoad = function interceptLoad() {
    ...
    function checkAsset(request, parent) {
        ...
        if (config && config.output && self.assetsCount > 0) {
            ...
            var assetUrl = self.assets.marked[x];
    ...
    function readAssets(err, configData) {
        ...
        try {
            ...
            if (!result.config.valid) {
                console.log(" > isomorphic-loader extend require: waiting for config file to be valid."); // eslint-disable-line
...
ExtendRequire.prototype.go = function go(callback) {
    ...
    if (!this.config && !fs.existsSync(configFile)) {
        ...
        setTimeout(function () {
            if (self.waitedForConfigDelay > 0) {
                console.log(" > isomorphic-loader extend require: config generated - starting."); // eslint-disable-line
            }
...
ExtendRequire.prototype.extend = function extend(userConfig, callback) {
    ...
    var startDelay = isNaN(this.userConfig.startDelay) ? 300 : this.userConfig.startDelay;
