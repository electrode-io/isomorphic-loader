...
ExtendRequire.prototype.interceptLoad = function interceptLoad() {
  ...
  const checkAsset = (moduleInstance, request, parent) => {
    ...
    if (config && config.output && this.assetsCount > 0) {
      ...
      let xRequest =
        loaderIndex > 0 ? request.substr(loaderIndex + 1) : request;
      ...
      try {
        resolved = posixify(
          removeCwd(moduleInstance._resolveFilename(xRequest, parent))
        );
      } catch (e) {
      ...
      if (assetUrl) {
        ...
        if (
          config.isWebpackDev &&
          config.webpackDev.addUrl &&
          config.webpackDev.url
        ) {
          const sep =
...
ExtendRequire.prototype._loadAssets = function _loadAssets(callback) {
        ...
        this._loadAssets((err2, res) => {
          if (!err2) {
            if (this.config && res.config.timestamp <= this.config.timestamp) {
              this._logConfigState(
                VALID_CONFIG_STATE,
                "skip reload. timestamp did not change"
              );
              return;
            }
  ...
  const handleWebpackDev = () => {
    if (
      !result.config.webpackDev.skipSetEnv &&
      !process.env.hasOwnProperty("WEBPACK_DEV")
    ) {
    ...
    if (!result.config.valid) {
      this._logConfigState(
        INVALID_CONFIG_STATE,
        "webpack dev server mode - waiting"
      );
...
ExtendRequire.prototype.waitingNotice = function waitingNotice() {
  ...
  if (now - this.waitStart >= this.delay) {
    this._logConfigState(
      "not ready",
      "waiting for config file to be generated."
    );
    this.waitStart = now;
    this.delay += 500;
    ...
    if (now - this.started >= Config.waitConfigTimeout) {
      ...
      logger.error(
        " >>>> please ensure webpack dev server is working properly."
      );
      return false;
    }
  }
