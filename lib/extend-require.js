    ...
    if (config && config.output && self.assetsCount > 0) {
      ...
      try {
        ...
        if (isAssetNotFound(xRequest)) {
          console.log(
            " > isomorphic-loader extend require: check asset " + xRequest + " exception",
            e
          ); // eslint-disable-line
        }
        return undefined;
...
ExtendRequire.prototype._loadAssets = function _loadAssets(callback) {
        ...
        self._loadAssets(function(err2, res) {
          if (!err2) {
            if (self.config && res.config.timestamp <= self.config.timestamp) {
              console.log(
                " > isomorphic-loader extend require: skip reload. timestamp did not change"
              ); // eslint-disable-line
              return;
            }
            ...
            console.log(
              " > isomorphic-loader extend require: file watcher load assets error",
              err2
            ); // eslint-disable-line
          }
        });
  ...
  function handleWebpackDev() {
    ...
    console.log(
      " > isomorphic-loader extend require: webpack dev server mode - config is now valid."
    ); // eslint-disable-line
