    ...
    const checkAsset = (moduleInstance, request, parent) => {
      ...
      if (config && config.output && this.assetsCount > 0) {
        ...
        if (assetUrl) {
          ...
          //allows mapping of "asset-file.png" into https://cdn.com/bucket/asset-file.png"
          if (config.cdnUrlMapping && config.cdnUrlMapping[basename(assetUrl)]) {
            assetUrl = config.cdnUrlMapping[basename(assetUrl)];
          }
