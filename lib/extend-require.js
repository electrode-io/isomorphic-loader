...
const checkCdnMapping = (config, assetUrl) => {
  if (cdnMapping.found[basename(assetUrl)]) return cdnMapping.found[basename(assetUrl)];
  if (cdnMapping.notFound[basename(assetUrl)]) return assetUrl;
  if (config.cdnUrlMappingFile && !cdnMapping.mapping) {
    cdnMapping.mapping = require(Path.resolve(config.cdnUrlMappingFile));
  }
  /* istanbul ignore next */
  if (!cdnMapping.mapping) return assetUrl;

  for (const key in cdnMapping.mapping) {
    if (key.includes(basename(assetUrl))) {
      cdnMapping.found[basename(assetUrl)] = cdnMapping.mapping[key];
      return cdnMapping.mapping[key];
    }
  }
  cdnMapping.notFound[basename(assetUrl)] = 1;
  return assetUrl;
};
class ExtendRequire {
  constructor() {
    ...
    const checkAsset = (moduleInstance, request, parent) => {
      ...
      if (config && config.output && this.assetsCount > 0) {
        ...
        if (assetUrl) {
          ...
          return checkCdnMapping(config, assetUrl);
        }
      }
  ...
  _loadAssets(callback) {
    ...
    const handleFileChangeEvent = event => {
      if (event !== "change") {
        logger.error(LOG_PREFIX, "unexpected config file watch event", event);
      } else {
        ...
        this.reloading = setTimeout(() => {
          ...
          this._loadAssets((err2, res) => {
            if (!err2) {
              ...
              logger.error(LOG_PREFIX, "file watcher load assets error", err2);
            }
          });
        }, Config.reloadDelay);
      }
  ...
  handleEventData(data) {
    if (data && data.name === Config.configName) {
      this.configString = data.config;
      this.loadAssets(err => {
        if (!err) {
          this.interceptLoad();
        } else {
          logger.error(LOG_PREFIX, "config event loadAssets failed\n", err);
        }
      });
    }
  }
...
extendRequire._checkCdnMapping = checkCdnMapping;

extendRequire.setLogLevel = level => logger.setLevel(level);
