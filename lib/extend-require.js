...
let originalLoad;
Module.cdnMap = {
  //hopefully static and in memory
    ...
    const checkAsset = (moduleInstance, request, parent) => {
      ...
      if (config && config.output && this.assetsCount > 0) {
        ...
        if (assetUrl) {
          ...
          const cdnMap = moduleInstance.cdnMap;

          cdnMap.config =
            cdnMap.config ||
            config.cdnUrlMapping ||
            (config.cdnUrlMappingFile && require(resolve(config.cdnUrlMappingFile)));

          if (cdnMap.notFound && cdnMap.notFound[assetUrl]) return assetUrl;
          if (cdnMap.found && cdnMap.found[assetUrl]) return self.cdnMap.found[assetUrl];

          cdnMap.hashing =
            cdnMap.hashing ||
            Object.keys(cdnMap.config).reduce([{}, {}, {}], function(hashing, filename, idx) {
              let rhs = "";
              for (let i = 0; i < 3; i++) {
                rhs += filename.substr(filename.lastIndexOf("/") + 1);
                hashing[i][rhs] = idx;
              }
            });
          ...
          let right = "";
          let left = assetUrl;
          let presumptiveIdx = -1;
          for (let i = 0; i < 3; i++) {
            const section = assetUrl.substr(assetUrl.lastIndexOf("/") + 1);
            if (!cdnMapExtended[i][section + right]) {
              cdnMap.notFound[assetUrl] = 1;
              break;
            } else {
              presumptiveIdx = idx;
              right += section;
          ...
          if (right !== "" && presumptiveIdx > -1) {
            cdnMap.found[assetUrl] = Object.values(cdnMap.config)[presumptiveIdx];
            return cdnMap.found[assetUrl];
          } else {
            cdnMap.notFound[assetUrl] = 1;
          }
          return assetUrl;
